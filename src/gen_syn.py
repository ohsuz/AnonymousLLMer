...
if args.data_type == "hf":
    ...
    output_dir = f"../data/{dataset_save_name}_{args.prompt_type}_{int(time.time())}.json"
    ...
    output_dir = f"{dataset_save_name}_{args.prompt_type}_{int(time.time())}.json"
...
for i in range(len(ranges)-1):
    ...
    for text in tqdm(cur_texts):
        match args.prompt_type:
            case "finance_lecture":
                user_prompt = input_finance_lecture(text)
            case "finance_article":
                user_prompt = input_finance_article(text)
            case "finance_wikipedia":
                user_prompt = input_finance_wikipedia(text)
